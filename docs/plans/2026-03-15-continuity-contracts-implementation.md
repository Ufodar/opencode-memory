...
**Goal:** 把 runtime 和 tool 层对具体 SQLite 实现的直接依赖，收紧成面向 continuity 领域的最小 contracts。

**Architecture:** 现有 `SQLiteContinuityStore` 已经把 SQLite 持久化层拆开，但上层仍直接引用 SQLite 具体类与 SQLite 目录内的类型。下一步把 continuity 的查询结果类型和最小 store interfaces 提升为 backend-agnostic contracts，让 runtime / injection / tools 依赖领域 contracts，而不是依赖具体 SQLite 名字。

**Tech Stack:** TypeScript, Bun, OpenCode plugin runtime, SQLite-backed persistence

---

### Task 1: 提升 continuity 结果类型为领域 contracts

**Files:**
- Create: `src/continuity/contracts.ts`
...
让 retrieval tools 的测试改为从 `src/continuity/contracts.ts` 引类型和最小接口。
...
- 新建 `src/continuity/contracts.ts`
- 把 `ContinuitySearchRecord`
...
- Modify: `notes/opencode-continuity.md`
...
```bash
git add README.md docs/architecture.md docs/plans/2026-03-15-continuity-contracts-implementation.md src tests
git commit -m "refactor: introduce backend-agnostic continuity contracts"
```
