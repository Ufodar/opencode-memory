...
验证 `opencode-memory` 不只是单元测试通过，而是真的能在本机 OpenCode 宿主中：

1. 加载本地 plugin
2. 暴露 memory tools
3. 在真实 `read` 后写入 observation
4. 在 `session.idle` 后写入 summary
5. 在后续请求里通过 `memory_search -> memory_timeline -> memory_details` 找回 memory

## 测试工作区

- 工作区：`/Users/storm/Documents/code/study_in_happy/downloads/opencode-memory-host-smoke`
- 局部配置：`.opencode/opencode.json`
- plugin 入口：`file:///Users/storm/Documents/code/study_in_happy/projects/opencode-memory/dist/index.js`
...
这不是 `opencode-memory` 的 bug，但会降低 smoke 结果的可读性。

## 当前判断

这轮真实宿主验证已经足够证明：

- `opencode-memory` 在真实 OpenCode 里可以形成完整 memory 主闭环
- 当前更大的集成噪声来自：
  - OpenCode 宿主的全局配置
  - model 指定方式
...
- 显式模型参数：
  - `--model my-company/Kimi-K2.5`
- 隔离 HOME：
  - `/Users/storm/Documents/code/study_in_happy/downloads/opencode-memory-host-smoke/.tmp-home-minimal`
...
- 未修改正式全局配置本身
- 最小宿主配置文件单独放在：
  - `/Users/storm/Documents/code/study_in_happy/downloads/opencode-memory-host-smoke/.tmp-opencode-minimal.json`
...
- 命令环境：
  ...
  - `--dir` 指向 smoke 工作区
- 第一次尝试失败，不是 memory bug，而是模型把 `./brief.txt` 漂移成了工作区外路径，触发：
  - `external_directory`
- 修正方式：
...
- 这说明之前 smoke 里的大量噪声，确实主要来自全局 MCP 和全局宿主配置，而不是 `opencode-memory` 自己
- 当前更值得继续观察的宿主变量变成：
...
- 这意味着：
  - 它不适合被当成当前 memory 主线问题
  - 也不适合被当成当前最值得投入的宿主 bug

### 这轮测试真正说明了什么

- `opencode-memory` 的 memory 主闭环，在最小宿主配置下是稳定成立的
...
- 当前 runner 已经具备：
  ...
  - 机器可读输出
  - 人类可读摘要
- 这意味着后面如果继续改 memory 主逻辑，先看 `report.md` 就足够判断“这次改动有没有把主闭环打坏”
